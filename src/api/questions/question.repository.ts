...
export class QuestionRepository {
	...
	async createQuestion(id: string, content: string, created_by: string): Promise<number> {
		const [result] = await db("questions").insert({ id, content, created_by });
		return result;
	}

	async updateQuestion(id: string, payload: Partial<Omit<Question, "id">>): Promise<number | null> {
