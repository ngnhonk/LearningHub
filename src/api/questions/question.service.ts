...
export class QuestionService {
	...
	async createQuestion(content: string, created_by: string): Promise<ServiceResponse<number | null>> {
		try {
			const id = uuidv7();
			const newQuestion = await this.questionRepository.createQuestion(id, content, created_by);
			return ServiceResponse.success<number>("Question created successfully", newQuestion);
		} catch (error) {
	...
	async updateQuestion(id: string, payload: Partial<Omit<Question, "id">>): Promise<ServiceResponse<Question | null>> {
		try {
			...
			return ServiceResponse.success<Question>("Question updated successfully", updatedQuestion as Question);
		} catch (error) {
